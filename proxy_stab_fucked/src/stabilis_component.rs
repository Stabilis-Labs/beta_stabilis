...
//! The STAB token is a stablecoin that is pegged to its own internal price, which is determined by an interest rate.
//!
//! To open a loan, users must provide collateral in the form of accepted tokens, and receive STAB tokens in return.
//! The collateral must be worth more than the STAB tokens received times a modifier (MCR), to ensure the loan is safe.
